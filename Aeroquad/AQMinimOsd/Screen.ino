...
void displayTemperature()        // WILL WORK ONLY WITH Rushduino V1.2
{
  int xxx = temperature;
...
void displayIntro()
{
  ...
  if (Settings[S_VIDEOSIGNALTYPE]) {
    MAX7456_WriteString_P(configMsgPAL, KVTeamVersionPosition+44);  // PAL
  }
  else {
...
void displayGPS_speed(void)
{
  ...
  int xx = GPS_speed * 0.02236932;      // (0.036*0.62137)  From MWii cm/sec to mph
  if(!Settings[S_UNITSYSTEM]) {
...
void displayDistanceToHome()
{
  ...
  int16_t dist = GPS_distanceToHome;                    // mt
  if(Settings[S_UNITSYSTEM]) {
